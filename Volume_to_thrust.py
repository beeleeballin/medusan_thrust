...
def main():
    ...
    plt.title("Correlation of Ejected Wake Volume and Jellyfish Swimming Thrust")
    plt.xlabel("Wake Volume m^3")
    plt.ylabel("Thrust g*m/s^2")
    figtext(0.15, 0.8, "r^2: %.2f" % np.power(r[0, 1], 2))
    figtext(0.15, 0.76, "line: %.3f x + %.2E" % (m, b))
    figtext(0.15, 0.72, "p-value at m: %.3E" % p_value[1])
    figtext(0.15, 0.68, "p-value at b: %.3E" % p_value[0])
    plt.tight_layout()
    plt.show()
