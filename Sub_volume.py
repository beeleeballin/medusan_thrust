...
######################################################################
# TASK 2:
# to make improvements to the model, a submodel that projects the total
# wake volume is necessary to understand thrust and acceleration.
# while the change in bell volume is a telling indication of the wake volume,
# prolate and oblate medusa eject different volumes of fluid per
# change in bell volume and the ejected volume form into wake volumes by
# a different factor. this submodel is constructed to make sound
# estimates for oblate medusan wake volume based the change of bell volume
# RESULT:
# a statistically significant correlation is found between the change of bell volume
# and change of wake volume,
######################################################################
def main():
    ...
    plt.ylabel("Change of Ejected Volume $(m^3)$")
    ...
    print(fii.summary())
    r = np.corrcoef(x, y)
    polymodel = np.poly1d(np.polyfit(x, y, 1))
    m, b = polymodel
